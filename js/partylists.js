...
// Partylists Logo + Poster File Uploads
function initializePartylistsFileUploads(root = document) {
    root.querySelectorAll(".partylists-file-upload").forEach(upload => {
        ...
        button.addEventListener("click", e => {
            ...
            if (existingFile) {
                showPartylistsFilePreview(upload, existingFile, input.classList.contains("partylists-logo-input"));
                return;
            }
        ...
        input.addEventListener("change", () => {
            ...
            const isLogo = input.classList.contains("partylists-logo-input");
            ...
            showPartylistsFilePreview(upload, file, isLogo);
...
function showPartylistsFilePreview(upload, file, isLogo) {
    const preview = upload.querySelector(".partylists-file-preview");
    const button = upload.querySelector(".partylists-file-btn");
    const input = upload.querySelector('input[type="file"]');
    ...
    const icon = file.type === "application/pdf"
        ? "bi-file-earmark-pdf"
        : isLogo ? "bi-image" : "bi-file-earmark-image";

    preview.innerHTML = `
        <div class="partylists-file-preview-header">
            <strong>${isLogo ? "Logo Preview" : "Poster Preview"}</strong>
            <button type="button" class="partylists-file-preview-close" title="Collapse Preview" aria-label="Collapse Preview">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
        ...
        <div class="partylists-file-preview-body">
            ...
            ? `<iframe src="${fileURL}" title="Partylists file preview"></iframe>`
            : `<img src="${fileURL}" alt="Partylists file preview">`}
        </div>

        <div class="partylists-file-preview-actions">
            <button type="button" class="partylists-file-change-btn"><i class="bi bi-arrow-repeat"></i>Change</button>
            <button type="button" class="partylists-file-delete-btn"><i class="bi bi-trash"></i>Delete</button>
        </div>
    `;

    preview.classList.add("show");

    preview.querySelector(".partylists-file-preview-close")?.addEventListener("click", e => {
        e.preventDefault();
        preview.classList.remove("show");
    });

    preview.querySelector(".partylists-file-change-btn")?.addEventListener("click", e => {
        e.preventDefault();
        input.click();
    });

    preview.querySelector(".partylists-file-delete-btn")?.addEventListener("click", e => {
        e.preventDefault();
        input.value = "";
        preview.innerHTML = "";
        preview.classList.remove("show");
...
function initializeEditMembers(card) {
    ...
    const members = [...card.querySelectorAll(".existing-member")].map((element, index) => {
        const spans = element.querySelectorAll("span");
        const name = (spans[1]?.textContent || "").trim();
        const parts = name.split(/\s+/);

        return {
            studentId: (element.dataset.studentId || spans[0]?.textContent || "").trim(),
            firstName: parts[0] || "",
            middleName: parts.length > 2 ? parts.slice(1, -1).join(" ") : "",
            lastName: parts.length > 1 ? parts[parts.length - 1] : "",
            position: (spans[2]?.textContent || DEFAULT_POSITIONS[index] || "Member").trim(),
    ...
    (members.length ? members : DEFAULT_POSITIONS.map(position => ({ position })))
...
function collectMemberData(row) {
    ...
    const position = positionSelect?.value === "Others"
    ...
    saveConfirm?.addEventListener("click", () => {
        ...
        setTimeout(() => {
            if (action.type === "create") {
                saveNewPartylists(action.form);
            } else if (action.type === "edit") {
                saveEditedPartylists(action.card);
            }
...
function saveEditedPartylists(card) {
    if (!card) return;
    ...
    const campusSelect = form.querySelector(".edit-partylists-campus");
    const title = card.querySelector(".partylists-item-title h3");
    const campusDisplay = card.querySelector(".partylists-campus span");
    const detailCampus = card.querySelector(".existing-partylists-campus");
    ...
    if (campusDisplay) campusDisplay.textContent = campus;
    if (detailCampus) detailCampus.textContent = campus;
    ...
    refreshExisting();
    showSuccessToast("Updated", `${name} was updated successfully.`);
}

function saveNewPartylists(form) {
    if (!form) return;

    const name = document.getElementById("partylistsName")?.value.trim() || "Untitled Partylists";
    const campus = document.getElementById("partylistsCampus")?.value || "College";
    const schoolYear = document.getElementById("partylistsSchoolYear")?.value.trim() || "Untitled School Year";
    const description = document.getElementById("partylistsDescription")?.value.trim() || "";
    const list = document.querySelector(".partylists-list");
    if (!list) return;

    const newCard = document.createElement("article");
    newCard.className = "partylists-item";
        ...
        <div class="partylists-item-header">
            <div class="partylists-item-title">
                <div class="partylists-logo-small">
                    <img class="partylists-logo-image" src="/images/default-partylists.png" alt="${name} Logo">
                    <span class="partylists-logo-initials">${getPartylistsInitials(name)}</span>
                </div>
                <div class="partylists-title-text">
                    <h3>${name}</h3>
                    <span class="partylists-description">${description || "Partylists created successfully."}</span>
                </div>
            </div>
            <div class="partylists-header-right">
                ...
                <div class="partylists-controls">
                    <button type="button" class="icon-btn edit-partylists" title="Edit"><i class="bi bi-pencil"></i></button>
                    <button type="button" class="icon-btn archive-partylists" title="Archive"><i class="bi bi-archive"></i></button>
                    <button type="button" class="icon-btn delete-partylists" title="Delete"><i class="bi bi-trash"></i></button>
                    <button type="button" class="icon-btn expand-partylists" title="Expand"><i class="bi bi-chevron-down"></i></button>
                </div>
            </div>
        </div>
            ...
            <div class="partylists-info">
                <div class="partylists-expanded-content">
                    <div class="partylists-poster-preview">
                        <div class="partylists-preview-title"><span>Poster</span></div>
                        <div class="partylists-file-preview existing-poster-preview"></div>
                    </div>
                    <div class="partylists-expanded-details">
                        <div class="partylists-info-row"><label>Campus:</label><span class="existing-partylists-campus">${campus}</span></div>
                        <div class="partylists-info-row">
                            <label>School Year:</label>
                            <span>${schoolYear}</span>
                        </div>
                        <div class="partylists-info-row members-info-row"><label>Members:</label><div class="existing-member-list"></div></div>
                    </div>
                </div>
            </div>
            <section class="edit-partylists-panel">
                <form class="partylists-form edit-partylists-form">
                    <div class="form-section">
                        <div class="form-group">
                            <label>Partylists Name</label>
                            <input type="text" value="${name}" class="edit-partylists-name-input">
                        ...
                        <div class="form-group">
                            <label>School Year</label>
                            <input type="text" value="${schoolYear}" class="edit-partylists-schoolyear-input">
                            <small class="inline-error edit-partylists-schoolyear-error"></small>
                        </div>
                        <div class="form-group">
                            <label>Campus</label>
                            <select class="edit-partylists-campus" required>
                                <option value="">Select Campus</option>
                                <option value="College" ${campus === "College" ? "selected" : ""}>College</option>
                                <option value="CBAS" ${campus === "CBAS" ? "selected" : ""}>CBAS</option>
                                <option value="Muzon" ${campus === "Muzon" ? "selected" : ""}>Muzon</option>
                                <option value="Francisco" ${campus === "Francisco" ? "selected" : ""}>Francisco</option>
                            </select>
                            <small class="inline-error edit-partylists-campus-error"></small>
                        </div>
                        <div class="form-group">
                            <label>Partylists Poster</label>
                            <div class="partylists-file-upload">
                                <input type="file" class="partylists-poster-input edit-poster-input" accept="image/*,.pdf,application/pdf" hidden>
                                <button type="button" class="partylists-file-btn"><i class="bi bi-file-earmark-image"></i><span>Change Poster</span></button>
                                <div class="partylists-file-preview"></div>
                            </div>
                        </div>
                        <div class="form-group">
                            <label>Partylists Logo <span class="optional-label">(Optional)</span></label>
                            <div class="partylists-file-upload">
                                <input type="file" class="partylists-logo-input edit-logo-input" accept="image/*" hidden>
                                <button type="button" class="partylists-file-btn"><i class="bi bi-image"></i><span>Change Logo</span></button>
                                <div class="partylists-file-preview"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-buttons">
                        <button type="button" class="discard-btn cancel-partylists-edit">Cancel</button>
    ...
    bindPartylistsCard(newCard);
    initializePartylistsFileUploads(newCard);
    bindEditForm(newCard);

    resetCreateForm();
    clearAllInlineErrors();
    showSuccessToast("Added", `${name} was added successfully.`);
}
...
function initializePosterUploads() {
    const inputs = document.querySelectorAll(".partylists-poster-input");

    inputs.forEach(input => {
        const upload = input.closest(".partylists-poster-upload");
        if (!upload) return;

        const button = upload.querySelector(".partylists-poster-btn");
        const nameDisplay = upload.querySelector(".partylists-poster-name");
        if (!button) return;
        ...
        input.addEventListener("change", () => {
            ...
            const card = upload.closest(".partylists-item");
            if (card) card._partylistsPosterFile = file;
